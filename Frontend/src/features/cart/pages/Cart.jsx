...
const Cart = () => {
  ...
  const handleRemove = async (item) => {
    try {
      ...
      alert(err.response?.data?.message || err.message || "Failed to remove item");
    }
  };
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="bg-white border border-[#EBE7DF] rounded-3xl p-6 shadow-sm space-y-6">
              ...
              <button className="w-full bg-[#1C1917] hover:bg-[#2C2927] text-white text-xs font-bold uppercase tracking-widest py-4 px-6 rounded-xl transition-all shadow-md active:scale-[0.98] mt-2">
                Proceed to Checkout
              </button>
